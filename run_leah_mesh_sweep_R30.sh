...
for HNM in "${HS[@]}"; do
  for CELLTYPE in hex tet; do
    ...
    # Rename the default fields.* files so they match the folder name exactly
    if [[ -f "${OUTDIR}/fields.h5" ]]; then
      mv "${OUTDIR}/fields.h5" "${OUTDIR}/${STEM}.h5"
    fi

    if [[ -f "${OUTDIR}/fields.xdmf" ]]; then
      mv "${OUTDIR}/fields.xdmf" "${OUTDIR}/${STEM}.xdmf"
    fi

    # Update XDMF so it points to the renamed H5 file
    python3 - <<PY
from pathlib import Path
p = Path("${OUTDIR}/${STEM}.xdmf")
if p.exists():
    txt = p.read_text()
    txt = txt.replace("fields.h5", "${STEM}.h5")
    p.write_text(txt)
...
PY
    ...
    echo
    echo "Finished:"
    echo "  ${OUTDIR}/${STEM}.xdmf"
    echo "  ${OUTDIR}/${STEM}.h5"
    echo
  done
done

echo "All R=30 nm mesh sweep runs completed."

...
# materials tags, optional asymmetric mesh-by-counts, and 2D slice grids.
import json
import numpy as np
...
def make_eps_r_two_layer(domain, epsr_top, epsr_bot):
    ...
    W = fem.functionspace(domain, ("Discontinuous Lagrange", 0))
    eps = fem.Function(W, name="epsilon_r")
...
def dirichlet_zero_top_bottom(V, ztol=1e-14):
    ...
    Z = dom.geometry.x[:, 2]
    zmin = dom.comm.allreduce(float(Z.min()), op=MPI.MIN)
    zmax = dom.comm.allreduce(float(Z.max()), op=MPI.MAX)
...
def _eval_func_at_points(u: fem.Function, Xq: np.ndarray):
    ...
    bad = cells < 0
    if np.any(bad):
        raise RuntimeError(f"Some slice points lie outside the mesh: {np.count_nonzero(bad)} points.")
...
def write_slice_xy(domain, uh, E_proj, z0, out_xdmf, name_suffix, nx2=200, ny2=200):
    ...
    xs = np.linspace(xmin, xmax, nx2)
    ys = np.linspace(ymin, ymax, ny2)
...
def write_slice_xz(domain, uh, E_proj, y0, out_xdmf, name_suffix, nx2=200, nz2=200):
    X = domain.geometry.x
    xmin, xmax = float(X[:,0].min()), float(X[:,0].max())
    zmin, zmax = float(X[:,2].min()), float(X[:,2].max())
    xs = np.linspace(xmin, xmax, nx2)
    zs = np.linspace(zmin, zmax, nz2)
...
def run(scene, Lx, Ly, H, h, eps1, eps2, q, a, sigma, deg, out, probe_axis,
    ...
    # 2D diagnostic slices (append to same XDMF)
    if domain.comm.rank == 0:
        print("[SLICE] writing z=d, z=0+, and y=0 slices")
    ...
    # analytic overlay for two-layer jump on the z-axis
...
def _parse():
    ...
    if args.use_counts:
        for k in ["Lx","Lz_top","Lz_bot","nx","ny","nz_top","nz_bot"]:
            if getattr(args, k) is None:
                raise SystemExit(f"--use_counts requires --{k}")
        # pick Ly: prefer Ly2, otherwise Ly, otherwise Lx
        if args.Ly2 is not None:
            Ly_counts = args.Ly2

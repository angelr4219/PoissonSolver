...
# Loop by mesh size first, then cell type
for HNM in "${HS[@]}"; do
  for CELLTYPE in hex tet; do
    ...
    OUTDIR="Results/leah_${CELLTYPE}_h${HNM}nm"
    BASENAME="fields_${CELLTYPE}_h${HNM}nm"
    ...
    # Rename fields files so each case has unique filenames
    ...
    # Update the XDMF metadata so it points at the renamed H5 file
    python3 - <<PY
...
echo "All mesh sweep runs completed."

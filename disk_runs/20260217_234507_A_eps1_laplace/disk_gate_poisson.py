...
# ---- version-proof LinearProblem import ----
try:
    from dolfinx.fem.petsc import LinearProblem  # newer/common
except Exception:
    try:
        from dolfinx.fem import LinearProblem  # some builds
    except Exception:
        LinearProblem = None
...
def solve_with_ksp(a_form, L_form, bcs, V, petsc_options):
    """
    Fallback solver if LinearProblem isn't available in this dolfinx build.
    """
    ...
    for k, v in petsc_options.items():
        ksp.setFromOptions()  # allow CLI options too
        # We'll set the most important ones explicitly below
    ksp.setOperators(A)

    # defaults
    ksp.setType(petsc_options.get("ksp_type", "cg"))
    pc = ksp.getPC()
    pc.setType(petsc_options.get("pc_type", "hypre"))
    ksp.setTolerances(rtol=petsc_options.get("ksp_rtol", 1e-10),
                      atol=petsc_options.get("ksp_atol", 1e-14),
                      max_it=petsc_options.get("ksp_max_it", 2000))
...
def main():
    ...
    if LinearProblem is not None:
        problem = LinearProblem(a, L, bcs=bcs, petsc_options=petsc_options)
        phi = problem.solve()
    else:
        # Last resort fallback (requires fem.petsc submodule)
        if not hasattr(fem, "petsc"):
            raise RuntimeError("No LinearProblem and no fem.petsc in this dolfinx build. Need a different image/version.")
        phi = solve_with_ksp(a, L, bcs, V, petsc_options)

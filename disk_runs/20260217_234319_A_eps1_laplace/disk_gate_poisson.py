...
def parse_args():
    p = argparse.ArgumentParser(
        description="3D Poisson/Laplace with a top-surface disk Dirichlet patch; optional dielectric stack + volumetric charge."
    )

    # Domain (meters)
    p.add_argument("--Lx", type=float, default=300e-9, help="Domain size in x (m)")
    p.add_argument("--Ly", type=float, default=300e-9, help="Domain size in y (m)")
    p.add_argument("--Lz", type=float, default=200e-9, help="Domain thickness in z (m). Domain is z in [-Lz, 0].")
    p.add_argument("--xc", type=float, default=0.0, help="Disk center x (m)")
    p.add_argument("--yc", type=float, default=0.0, help="Disk center y (m)")

    # Mesh
    p.add_argument("--h", type=float, default=10e-9, help="Target mesh size (m), used to pick nx,ny,nz")
    p.add_argument("--deg", type=int, default=1, help="CG polynomial degree for phi")

    # Disk gate BC
    p.add_argument("--R", type=float, default=50e-9, help="Disk radius (m)")
    p.add_argument("--Vdisk", type=float, default=0.1, help="Disk voltage (V)")

    # Dielectric model
    p.add_argument("--tox", type=float, default=0.0, help="Top oxide thickness (m). If 0, uniform dielectric.")
    p.add_argument("--eps_ox", type=float, default=3.9, help="Relative permittivity of oxide layer")
    p.add_argument("--eps_si", type=float, default=11.7, help="Relative permittivity of semiconductor (Si)")
    p.add_argument("--eps_uniform", type=float, default=None,
                   help="If set, ignore tox/eps_ox/eps_si and use uniform relative permittivity eps_uniform")

    # Charge model (volumetric Gaussian)
    p.add_argument("--rho0", type=float, default=0.0, help="Peak volumetric charge density (C/m^3). 0 => Laplace.")
    p.add_argument("--xq", type=float, default=0.0, help="Charge center x (m)")
    p.add_argument("--yq", type=float, default=0.0, help="Charge center y (m)")
    p.add_argument("--zq", type=float, default=-60e-9, help="Charge center z (m)")
    p.add_argument("--sigma", type=float, default=15e-9, help="Gaussian sigma (m)")

    # Outputs
    p.add_argument("--outdir", type=str, default="results_disk", help="Output directory")
    p.add_argument("--basename", type=str, default="disk", help="Base name for output files")
    p.add_argument("--probe_n", type=int, default=201, help="Number of probe points along z at (xc,yc)")
    return p.parse_args()
...
def locate_facets(mesh3d, Lx, Ly, Lz, xc, yc, R):
    """
    Return facet index arrays for:
      - disk (top, inside radius)
      - top_out (top, outside disk)
      - bottom
      - sides
    """
    ...
    def on_top(x):
        return near(x[2], zmax, tol)

    def on_bottom(x):
        return near(x[2], zmin, tol)

    def on_sides(x):
        return (near(x[0], xmin, tol) | near(x[0], xmax, tol) | near(x[1], ymin, tol) | near(x[1], ymax, tol))
...
def build_epsilon(mesh3d, tox, eps_ox, eps_si, eps_uniform):
    """
    Cellwise absolute permittivity eps_abs (F/m) as DG0.
    If eps_uniform not None: eps_abs = eps_uniform*EPS0 everywhere.
    Else: two-layer in z with top oxide thickness tox (z in [-tox, 0]).
    """
...
def main():
    ...
    # IMPORTANT: attach measures to this mesh
    ...
    if comm.rank == 0:
        ...
        if args.rho0 != 0.0:
            print(f"Charge: rho0={args.rho0:.3e} C/m^3, center=({args.xq:.3e},{args.yq:.3e},{args.zq:.3e}), sigma={args.sigma:.3e}")
        else:
            print("Charge: rho0=0 (Laplace)")
    ...
    if comm.rank == 0:
        ...
        if len(dofs_disk) == 0:
            raise RuntimeError("Disk facet selection failed (0 dofs). Check R/h/domain.")

    bc_disk = fem.dirichletbc(PETSc.ScalarType(args.Vdisk), dofs_disk, V)
    bc0_top = fem.dirichletbc(PETSc.ScalarType(0.0), dofs_top_out, V)
    bc0_bottom = fem.dirichletbc(PETSc.ScalarType(0.0), dofs_bottom, V)
    bc0_sides = fem.dirichletbc(PETSc.ScalarType(0.0), dofs_sides, V)
    bcs = [bc_disk, bc0_top, bc0_bottom, bc0_sides]

    eps = build_epsilon(domain, args.tox, args.eps_ox, args.eps_si, args.eps_uniform)

    x = ufl.SpatialCoordinate(domain)
    if args.rho0 == 0.0:
        rho = PETSc.ScalarType(0.0)
    else:
        r2 = (x[0] - args.xq)**2 + (x[1] - args.yq)**2 + (x[2] - args.zq)**2
        rho = PETSc.ScalarType(args.rho0) * ufl.exp(-r2 / (2.0 * args.sigma**2))
    ...
    if comm.rank == 0:
        ...
        np.savetxt(probe_path, np.column_stack([zvals, phivals]), header="z (m)    phi(xc,yc,z) (V)")

...
# 1) Run the rectangular-gate benchmark exactly like before
...
# Move ALL phi/artifacts into the rect folder (safe if some don’t exist)
shopt -s nullglob
for f in results/phi_p*a*; do mv -f "$f" "${RECT_DIR}/"; done
shopt -u nullglob

# 2) Create (or update) scripts/exact_rect_sweep.py with your code, then run it
mkdir -p scripts
...
# Run exact sweep inside Docker (env-driven)
...
# Move the produced CSV into the exact folder with the stamp

...
# Writes: phi (CG1) and eps_r (DG0) to XDMF for ParaView.
...
def eps_r_DG0(domain, z_split, epsr_top, epsr_bottom):
    """
    Return DG0 relative-permittivity field eps_r(z):
      z > z_split  -> epsr_top
      z <= z_split -> epsr_bottom
    Uses DG0.interpolate (no topology connectivity tricks).
    """
    W = fem.functionspace(domain, ("Discontinuous Lagrange", 0))
    epsr = fem.Function(W, name="eps_r")
    def _epsr_at_dofs(X):
        # X shape: (3, N) at DG0 quad points (one per cell)
        return np.where(X[2] > z_split, float(epsr_top), float(epsr_bottom)).astype(float)
    epsr.interpolate(_epsr_at_dofs)
    return epsr

def rho_gaussian_3d(domain, q, x0, y0, z0, sigma):
    """
    Normalized 3D Gaussian charge density centered at (x0,y0,z0):
...
def solve(Lx, Ly, Lz, h, z_split, epsr_top, epsr_bottom,
    ...
    # Dirichlet phi=0 on all outer faces
    ...
    # Coefficient and RHS
    epsr = eps_r_DG0(msh, z_split, epsr_top, epsr_bottom)  # DG0
    eps  = epsr * PETSc.ScalarType(EPS0)                    # DG0 * scalar
    rho  = rho_gaussian_3d(msh, q, x0, y0, z0, sigma)       # UFL expression
    ...
    with io.XDMFFile(comm, str(xdmf), "w") as xf:
        xf.write_mesh(msh)
        # Save phi as CG1 for smooth viewing
...
def parse_args():
    p = argparse.ArgumentParser(description="3D split-dielectric + point charge (grounded box)")

...
from dolfinx.fem.petsc import LinearProblem  # dolfinx 0.10.0
...
def main():
    ...
    # IMPORTANT: RHS stays a valid Form even when rho0=0 (dolfinx 0.10.0 needs this)
    rho0 = fem.Constant(domain, PETSc.ScalarType(args.rho0))
    X = ufl.SpatialCoordinate(domain)
    r2 = (X[0] - args.xq)**2 + (X[1] - args.yq)**2 + (X[2] - args.zq)**2
    rho = rho0 * ufl.exp(-r2 / (2.0 * args.sigma**2))
    ...
    # FORCE HDF5-backed XDMF for ParaView
    xdmf_path = os.path.join(args.outdir, f"{args.basename}.xdmf")
    with io.XDMFFile(comm, xdmf_path, "w", encoding=io.XDMFFile.Encoding.HDF5) as xdmf:
